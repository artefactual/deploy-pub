node {
  timestamps {
    stage('Get code') {
      ...
      env.AM_BRANCH = sh(script: 'echo ${AM_BRANCH:-"stable/1.14.x"}', returnStdout: true).trim()
      env.AM_VERSION = sh(script: 'echo ${AM_VERSION:-"1.14"}', returnStdout: true).trim()
      env.SS_BRANCH = sh(script: 'echo ${SS_BRANCH:-"stable/0.20.x"}', returnStdout: true).trim()
