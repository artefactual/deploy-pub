node {
  timestamps {
    stage('Get code') {
      ...
      env.AM_BRANCH = sh(script: 'echo ${AM_BRANCH:-"stable/1.13.x"}', returnStdout: true).trim()
      env.AM_VERSION = sh(script: 'echo ${AM_VERSION:-"1.13"}', returnStdout: true).trim()
      env.SS_BRANCH = sh(script: 'echo ${SS_BRANCH:-"stable/0.18.x"}', returnStdout: true).trim()
